...
@Service("tikiboxTextService")
public class TikiboxTextService {
	...
	public String process(BaseMsgDto baseMsgDto) {
		...
		ReplyKeyPojo replyKey = tikiBoxContentService.reply(textMsgDto.getContent());
		if (replyKey != null) {
			if (replyKey.getReplyType() == 0) {
				...
				String s = String
						.format(RESPONSE_TXT, textMsgDto.getFromUserName(), textMsgDto.getToUserName(),
								System.currentTimeMillis(), "text", tikiBoxContentService.preface());
				return s;
		}
