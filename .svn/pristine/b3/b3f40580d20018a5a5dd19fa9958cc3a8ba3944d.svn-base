...
var editor1;
	...
	function saveinfor(){  
		...
		$('#inforForm').form('submit',{  
		    url: 'doeditarticle.htm',  
		    onSubmit: function(){  
		        return $(this).form('validate');  
		    },  
		    success: function(result){
		        if (result==1){  
		             $.messager.confirm('提示','<span style="color:blue;">保存成功！是否跳转到列表页面?</span>',function(r){ 
		        	 ...
		        	 $.messager.show({  
		                 title: '提示',  
		                 msg: '<span style="color:red;">保存失败</span>'
		             });  
		        }  
		    }  
		});  
				...
				<div title="文章详细">
					<dl class="form_dl  clearfix">
						...
						<input type="hidden" value="${obj.id}" name="id"/>
							<input class="text_300 easyui-validatebox" name="title" value="${obj.title}" type="text"
						...
						<dt>资讯内容：</dt>
						<dd>
							<textarea class="ff_textarea"  name="content">${obj.content}</textarea>
						</dd>
					</dl>
				</div>
