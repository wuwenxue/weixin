...
<script charset="utf-8" src="js/kindeditor/lang/zh_CN.js"></script>
    ...
    <c:forEach begin="1" end="${num}" var="v" varStatus="s">
	var editor${s.current} = null;
	</c:forEach>

    function submitForm(){
    	$.messager.progress();
    	 <c:forEach begin="1" end="${num}" var="v" varStatus="s">
 		editor${s.current}.sync();
 		</c:forEach>
    	$('#form').form('submit', {
    		onSubmit: function(){
    ...
    function numFun(){
    	var num = $("#num").val();
    	$("#formNum").val(num);
    	window.location = 'materialAdd.html?num='+num;
    }
	...
	<div class="easyui-layout" data-options="fit:true">
            ...
            <div data-options="region:'center',border:false" style="padding:10px;">
               ...
               <div class="content" style="width: 1100px;">
				<form id="form" action="ajaxMaterialAddDo.html" method="post" >
				<input type="hidden" name="num" id="formNum" value="${num}">
				<input type="hidden" name="plat" id="plat" value="${plat}">
				<c:forEach begin="1" end="${num}" var="v" varStatus="s">
					<div id="myDiv" class="<c:if test="${s.current%2!=0}">left</c:if><c:if test="${s.current%2==0}">right</c:if> msg-editer" >
					<label class="block msg-editer-text" for="">标题${s.current}</label> 
					<input name="title${s.current}" class="msg-input easyui-validatebox" type="text" data-options="required:true"/> 
